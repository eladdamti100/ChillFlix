            ...
            if (!responseSent) { // Ensure response is sent only once
                if (response.startsWith("400")) {
                    ...
                    if (response.includes("\n")) {
                        ...
                        const movieIds = moviesData.split(" ").map(movie => movie.trim()); 
                        
                        res.status(200).json({ MovieRecommendations: movieIds });
                    
                    } else res.status(200).json({MovieRecommendations: null}) // no movies to recommend
